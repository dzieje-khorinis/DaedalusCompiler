...
namespace Common.Zen
{
    ...
    public class ZenLoader
    {
        ...
        private List<string[]> _filesContentsLines = new List<string[]>();
        ...
        public int Load()
        {
            ...
            {
                if (verbose) Console.WriteLine($"[{i + 1}/{_zenPaths.Count}]Parsing ZEN: {_zenPaths[i]}");
        ...
        public int Load(string zenContent)
        {
            ...
            if (verbose) Console.WriteLine($"[1/1]Parsing ZEN: test.zen");
            _parseTrees.Add(Parse(zenContent));
            try
            {
                HandleSyntaxErrors();
        ...
        private static List<string> GetFilesInsensitive(string dirPath, string filenamePattern)
        {
            ...
            {
                throw new FileNotFoundException($"ZenLoader ERROR: Could not find any files in '{dirPath}' that matches pattern '{filenamePattern}'");
            }
        ...
        private static string GetFileInsensitive(string dirPath, string filenamePattern)
        {
            ...
            {
                throw new DirectoryNotFoundException($"ZenLoader ERROR: Ambiguous path '{Path.Combine(dirPath, filenamePattern)}'. Matches {String.Join(";", filePaths)}");
            }
        ...
        private static List<string> LoadFilePaths(List<string> zenPaths)
        {
            ...
            foreach (string path in zenPaths)
            {
                ...
                {
                    throw new Exception($"ZenLoader ERROR: invalid file format '{pathExtensionLower}', required '.zen'");
                }
            }
            return result;
        }
